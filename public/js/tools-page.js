...
function renderKeys(keys) {
  ...
  if (!keys.length) return body.append(el('tr', {}, [el('td', { colSpan: 7, className: 'empty-cell', text: 'No API keys found.' })]));
  for (const key of keys) {
    ...
    const name = labeledCell('Name / Key', [el('div', { className: 'cell-primary', text: key.key_name }), el('div', { className: 'cell-secondary mono', text: key.api_key })]);
    body.append(el('tr', {}, [textCell(key.id || key.key_id, 'mono', 'ID'), name, textCell(key.status, '', 'Status'), textCell(key.request_count ?? 0, '', 'Requests'), textCell(key.last_used_at, '', 'Last used'), textCell(key.created_at, '', 'Created'), actions]));
  }
}
...
async function loadKeys() {
  ...
  try { const response = await apiFetch(`/api/keys?accountIndex=${encodeURIComponent(accountIndex)}`); renderKeys(response.data.keys || []); showStatus(status, `Loaded ${response.data.count ?? response.data.keys?.length ?? 0} API key(s).`, 'success'); }
...
function renderQuota(quota) {
  ...
  document.querySelector('#quota-percent').textContent = total > 0 ? `${percent}% used` : 'No capacity data';
...
function renderEligible(items) {
  const body = document.querySelector('#eligible-body'); const select = document.querySelector('#upgrade-subdomain-id'); clear(body); clear(select).append(el('option', { value: '', text: items.length ? 'Select an eligible subdomain' : 'No eligible subdomains' }));
  if (!items.length) return body.append(el('tr', {}, [el('td', { colSpan: 4, className: 'empty-cell', text: 'No eligible subdomains reported.' })]));
  for (const item of items) {
    ...
    select.append(el('option', { value: id, text: `${domain || `Subdomain ${id}`} (#${id})` }));
    const actions = el('td', { className: 'actions', 'data-label': 'Action' }, [button('Select', { className: 'btn btn--quiet', 'data-action': 'select-eligible', 'data-id': id })]);
    body.append(el('tr', {}, [textCell(id, 'mono', 'ID'), textCell(domain, '', 'Domain'), textCell(item.status || 'eligible', '', 'Status'), actions]));
  }
}
function renderAssistLogs(items) {
  const body = document.querySelector('#assist-logs-body'); clear(body);
  if (!items.length) return body.append(el('tr', {}, [el('td', { colSpan: 5, className: 'empty-cell', text: 'No assist activity.' })]));
  for (const item of items) body.append(el('tr', {}, [textCell(item.id || item.log_id, 'mono', 'ID'), textCell(item.assist_code || item.request_id, 'mono', 'Code / Request'), textCell(item.status, '', 'Status'), textCell(item.account || item.user || item.assisted_by, '', 'Account / User'), textCell(item.created_at, '', 'Created')]));
}
function renderUpgrades(items) {
  const body = document.querySelector('#upgrades-body'); clear(body);
  if (!items.length) return body.append(el('tr', {}, [el('td', { colSpan: 6, className: 'empty-cell', text: 'No permanent-upgrade requests.' })]));
  for (const item of items) {
    const requestId = item.request_id || item.id; const actions = el('td', { className: 'actions', 'data-label': 'Actions' });
    actions.append(button('Assist', { className: 'btn btn--quiet', 'data-action': 'assist-upgrade', 'data-id': requestId })); actions.append(button('Cancel', { className: 'btn btn--danger', 'data-action': 'cancel-upgrade', 'data-id': requestId }));
    body.append(el('tr', {}, [textCell(requestId, 'mono', 'Request'), textCell(item.full_domain || item.domain || item.subdomain_id, '', 'Subdomain'), textCell(item.status, '', 'Status'), textCell(item.created_at, '', 'Created'), textCell(item.updated_at, '', 'Updated'), actions]));
  }
}
function renderUpgradePagination() { document.querySelector('#upgrade-page-label').textContent = `Page ${upgrades.page}`; document.querySelector('#upgrade-previous').disabled = upgrades.page <= 1; document.querySelector('#upgrade-next').disabled = !upgrades.hasMore; }
...
function renderWhois(value) {
  ...
  const fields = [['Domain', value.domain], ['Registered', value.registered === false ? 'No' : 'Yes'], ['Status', value.status], ['Registered at', value.registered_at], ['Expires at', value.expires_at], ['Registrant email', value.registrant_email], ['Nameservers', value.nameservers || value.name_servers], ['Rate limit', value.rate_limit ? `${value.rate_limit.remaining}/${value.rate_limit.limit} remaining; reset ${value.rate_limit.reset_at || 'unknown'}` : 'Not reported']];
...
function installTools() {
  ...
  document.querySelector('#keys-body').addEventListener('click', async (event) => { const target = event.target.closest('button'); const action = target?.dataset.action; if (!action) return; const keyId = target.dataset.id; const accountIndex = selectedAccount(selector); if (action === 'delete') { if (!await confirmAction({ title: 'Delete API key', message: `Delete API key #${keyId} permanently?`, confirmLabel: 'Delete key' })) return; try { const result = await withLoading(target, () => apiFetch('/api/keys', { method: 'DELETE', body: JSON.stringify({ accountIndex, key_id: Number(keyId) }) })); showResult('API key deleted', result.data.message || `Key #${keyId} deleted.`, result.data, target); await loadKeys(); } catch (error) { toast(errorMessage(error), 'error'); } } else if (action === 'regenerate') { if (!await confirmAction({ title: 'Regenerate API key', message: 'The existing API secret will stop working. Continue?', confirmLabel: 'Regenerate' })) return; try { const result = await withLoading(target, () => apiFetch('/api/keys', { method: 'POST', body: JSON.stringify({ action: 'regenerate', accountIndex, key_id: Number(keyId) }) })); showSecret(result.data, target); toast(result.data.warning || 'API key regenerated. Store its new secret now.', 'success'); await loadKeys(); } catch (error) { toast(errorMessage(error), 'error'); } } });
  document.querySelector('#whois-form').addEventListener('submit', async (event) => { event.preventDefault(); const form = event.currentTarget; const values = new FormData(form); const domain = values.get('domain'); const mode = values.get('mode'); try { const response = await withLoading(form.querySelector('[type="submit"]'), () => apiFetch(`/api/whois?accountIndex=${encodeURIComponent(selectedAccount(selector))}&mode=${encodeURIComponent(mode)}&domain=${encodeURIComponent(domain)}`)); renderWhois(response.data.whois); document.querySelector('#whois-auth-mode').textContent = response.data.authMode === 'public' ? 'Queried with DNSHE public WHOIS mode.' : 'Queried with the selected DNSHE account.'; } catch (error) { document.querySelector('#whois-fields').hidden = true; document.querySelector('#whois-auth-mode').textContent = 'WHOIS lookup failed.'; document.querySelector('#whois-result').textContent = errorMessage(error); } });
  ...
  document.querySelector('#upgrade-form').addEventListener('submit', async (event) => { event.preventDefault(); const form = event.currentTarget; const id = Number(new FormData(form).get('subdomain_id')); if (!await confirmAction({ title: 'Request permanent upgrade', message: `Request permanent upgrade for subdomain #${id}? This can affect quota and may be irreversible.`, confirmLabel: 'Request upgrade' })) return; try { const result = await withLoading(form.querySelector('[type="submit"]'), () => apiFetch('/api/permanent_upgrade', { method: 'PUT', body: JSON.stringify({ action: 'create', accountIndex: selectedAccount(selector), subdomain_id: id }) })); showResult('Permanent upgrade requested', result.data.message || `Upgrade requested for subdomain #${id}.`, result.data, form.querySelector('[type="submit"]')); form.reset(); await loadUpgrades(); } catch (error) { showStatus(form.querySelector('.form-status'), errorMessage(error), 'error'); } });
  document.querySelector('#upgrades-body').addEventListener('click', async (event) => { const target = event.target.closest('button'); const action = target?.dataset.action; if (!action) return; const request_id = target.dataset.id; if (action === 'cancel-upgrade') { if (!await confirmAction({ title: 'Cancel permanent upgrade', message: `Cancel permanent-upgrade request #${request_id}?`, confirmLabel: 'Cancel request' })) return; try { const result = await withLoading(target, () => apiFetch('/api/permanent_upgrade', { method: 'DELETE', body: JSON.stringify({ accountIndex: selectedAccount(selector), request_id }) })); showResult('Upgrade request cancelled', result.data.message || `Request #${request_id} cancelled.`, result.data, target); await loadUpgrades(); } catch (error) { toast(errorMessage(error), 'error'); } } else { const form = document.querySelector('#assist-form'); form.reset(); clearStatus(form.querySelector('.form-status')); openDialog(document.querySelector('#assist-dialog'), target); } });
  ...
  document.querySelector('#assist-form').addEventListener('submit', async (event) => { event.preventDefault(); const form = event.currentTarget; const values = Object.fromEntries(new FormData(form).entries()); if (!await confirmAction({ title: 'Submit assist code', message: 'Submit this assist code for the selected account?', confirmLabel: 'Submit code' })) return; try { const result = await withLoading(form.querySelector('[type="submit"]'), () => apiFetch('/api/permanent_upgrade', { method: 'POST', body: JSON.stringify({ action: 'assist', accountIndex: selectedAccount(selector), assist_code: values.assist_code }) })); closeDialog(document.querySelector('#assist-dialog')); showResult('Assist code submitted', result.data.message || 'Assist operation completed.', result.data, document.querySelector('#open-assist')); await loadUpgrades(); } catch (error) { showStatus(form.querySelector('.form-status'), errorMessage(error), 'error'); } });
...
async function initializeSafely() { try { await initialize(); } catch (error) { showStatus(document.querySelector('#tools-status'), `Signed in, but the tools page could not initialize: ${errorMessage(error)}`, 'error'); toast(errorMessage(error), 'error'); } }
const auth = installAuthentication({ onAuthenticated: initializeSafely });
refreshSession().then(initializeSafely).catch((error) => { if (error?.status === 401) auth.requireLogin(); else showStatus(document.querySelector('#tools-status'), `Unable to check the current session: ${errorMessage(error)}`, 'error'); });
